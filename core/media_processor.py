...
async def transcribe_audio_bytes(file_bytes: bytes, filename: str, ext: str) -> str:
    ...
    if api_key:
        ...
        # 策略 1: dashscope whisper（最稳定，45s 超时）
        try:
            import openai as _oai, httpx
            _wc = _oai.AsyncOpenAI(api_key=api_key,
                base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",
                timeout=httpx.Timeout(50.0, connect=10.0))
            result = await asyncio.wait_for(
                _wc.audio.transcriptions.create(model="whisper-1", file=(filename, file_bytes, "audio/mpeg")),
                timeout=45)
            if result.text.strip():
                logger.info("dashscope whisper bytes STT: %s → %d 字", filename, len(result.text))
                return result.text.strip()
        except asyncio.TimeoutError:
            logger.warning("dashscope whisper bytes 超时(45s)")
        except Exception as e:
            logger.warning("dashscope whisper bytes STT 失败: %s", e)
        # 策略 2: Qwen-Omni（60s 超时）
        try:
            import openai, httpx
...
async def _qwen_omni_stt(path: Path, prompt: str = "请将这段音频内容完整转录为文字") -> dict:
    ...
    try:
        ...
        client = openai.AsyncOpenAI(api_key=api_key,
            ...
            timeout=httpx.Timeout(90.0, connect=10.0))
        model = os.environ.get("OMNI_MODEL", "qwen3-omni-flash")
        logger.info("Qwen-Omni STT 开始: file=%s, size=%d, model=%s", path.name, path.stat().st_size, model)
        ...
        text = await asyncio.wait_for(_do_stt(), timeout=60)
        if text:
        ...
        logger.warning("Qwen-Omni STT 超时(60s): %s", path.name)
        return {"text": "", "source": "qwen-omni", "success": False, "error": "STT 超时(60s)"}
    except Exception as e:
...
async def stt_transcribe(path: Path, prompt: str = "请将这段音频内容完整转录为文字") -> dict:
    """音频转文字。优先 dashscope whisper（稳定）→ Qwen-Omni → 提示配置。"""
    api_key = os.environ.get("DASHSCOPE_API_KEY", "")
    if api_key:
        # 策略 1: dashscope whisper（最稳定，45s 超时）
        try:
            text = await asyncio.wait_for(_dashscope_stt(path, api_key), timeout=45)
            if text:
                return {"text": text, "source": "dashscope-whisper", "success": True}
        except asyncio.TimeoutError:
            logger.warning("dashscope whisper 超时(45s)")
        except Exception as e:
            logger.warning("dashscope whisper STT 失败: %s", e)
        # 策略 2: Qwen-Omni（降级，60s 超时）
        try:
            result = await asyncio.wait_for(_qwen_omni_stt(path, prompt), timeout=65)
            if result["success"]:
                return result
            logger.warning("Qwen-Omni STT 失败: %s", result.get("error", ""))
        except asyncio.TimeoutError:
            logger.warning("Qwen-Omni STT 超时(65s)")
        except Exception as e:
