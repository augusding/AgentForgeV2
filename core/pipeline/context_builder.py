...
class ContextBuilder:
    ...
    def _build_output_guide(self, position: PositionConfig) -> str:
        ...
        return (
            ...
            "- 中文回复，必要时用 Markdown\n\n"
            "### 工具使用原则\n"
            "1. 查询待办/日程/跟进 → 调用对应工具获取数据\n"
            "2. 创建/修改/删除 → 调用对应工具执行\n"
            "3. 需要搜索 → search_knowledge 或 web_search\n"
            "4. 文件格式转换 → document_converter\n"
            "5. 如果需要生成 Word/Excel/PPT 文件，调用 word_processor/excel_processor 等工具\n"
            "6. 工具执行后简要确认即可\n"
        )
