...
class ContextBuilder:
    ...
    def _build_output_guide(self, position: PositionConfig) -> str:
        ...
        return (
            ...
            "- 中文回复，必要时用 Markdown\n\n"
            "### 工具使用原则（严格遵守）\n"
            "⚠️ 以下规则必须严格执行，违反将导致用户收到错误信息：\n"
            "1. 查询待办/日程/跟进 → 必须调用工具\n"
            "2. 创建/修改/删除 → 必须调用工具\n"
            "3. 生成任何文件 → 必须调用对应工具：\n"
            "   Word/报告/方案/邮件/纪要 → word_processor(content=文档内容)\n"
            "   Excel/表格/数据 → excel_processor(action=create)\n"
            "   PPT/演示文稿 → document_converter(target_format=pptx)\n"
            "   格式转换 → document_converter\n"
            "   未指定格式 → 默认 word_processor\n"
            "4. 搜索知识库 → search_knowledge\n"
            "5. 联网搜索 → web_search\n"
            "6. 【绝对禁止】不调用工具就声称已创建文件\n"
            "7. 【绝对禁止】在回复中编造文件路径，路径只能来自工具返回\n"
            "8. 不确定是否需要工具 → 就调用工具\n"
            "9. 工具执行后简要确认即可\n"
        )
